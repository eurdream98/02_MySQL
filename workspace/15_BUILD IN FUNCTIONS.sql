...
-- BIT_LENGTH(문자열) : 할당 된 비트 크기 반환, CHAR_LENGTH(문자열) : 문자열의 길이 반환, LENGTH(문자열) : 할당 된 byte 크기 반환
...
SELECT
 ...
 FIND_IN_SET('바나나', '사과,딸기,바나나'), INSTR('사과딸기바나나', '딸기'),
LOCATE('딸기', '사과딸기바나나');

-- FORMAT(숫자, 소수점 자릿수) : 1000단위마다 콤마(,) 표시를 해 주며 소수점 아래 자릿수(반올림)까지 표현한다.
...
-- UPPER(문자열) : 대문자로 변경, LOWER(문자열) : 소문자로 변경
SELECT LOWER('Hello World!'), UPPER('Hello World!');

-- LPAD(문자열, 길이, 채울 문자열) : 문자열을 길이만큼 왼쪽으로 늘린 후에 빈 곳을 문자열로 채운다.
-- RPAD(문자열, 길이, 채울 문자열) : 문자열을 길이만큼 오른쪽으로 늘린 후에 빈 곳을 문자열로 채운다.
SELECT LPAD('왼쪽', 1, '@'), RPAD('오른쪽',1  ,'@');

-- LTRIM(문자열) : 왼쪽 공백 제거, RTRIM(문자열) : 오른쪽 공백 제거
SELECT LTRIM('        왼쪽'), RTRIM('오른쪽        ');

-- TRIM(문자열) : 양쪽 공백 제거
-- TRIM(방향 자를_문자열 FROM 문자열) : 방향(LEADING(앞), BOTH(양쪽), TRAILING(뒤))이 있으면 해당 방향에 지정한 문자열을 제거할 수 있다.
...
-- 구분자가 왼쪽부터 횟수 번째 나오면 그 이후의 오른쪽은 버린다. 횟수가 음수일 경우 오른쪽부터 세고 왼쪽을 버린다.
SELECT
...
-- MOD(숫자1, 숫자2) 또는 숫자1 % 숫자2 또는 숫자1 MOD 숫자2 : 숫자 1을 숫자 2로 나눈 나머지 추출
SELECT MOD(75, 10), 75 % 10, 75 MOD 10;

-- POW(숫자1, 숫자2) : 거듭제곱값 추출, SQRT(숫자) : 제곱근 추출
SELECT POW(2, 4), SQRT(16);

-- RAND() : 0 이상 1 미만의 임의의 실수를 구한다.
...
-- CURDATE() : 현재 연-월-일 추출, CURTIME() : 현재 시:분:초 추출
...
-- YEAR(날짜), MONTH(날짜), DAYOFMONTH(날짜), HOUR(시간), MINUTE(시간), SECOND(시간), MICROSECOND(시간)
...
-- DATE() : 연-월-일만 추출, TIME() : 시:분:초만 추출
SELECT NOW(), DATE(NOW()), TIME(NOW());

-- DATEDIFF(날짜1, 날짜2) : 날짜1 - 날짜2의 일수를 반환
-- TIMEDIFF(날짜1 또는 시간1, 날짜1 또는 시간2) : 시간1 - 시간2의 시간을 반환
SELECT DATEDIFF('2024-01-30', NOW()), TIMEDIFF('17:07:11', '13:06:10');
...
-- LAST_DAY(날짜) : 해당 날짜의 달에서 마지막 날의 날짜를 구한다.
...
-- PERIOD_ADD(연월, 개월 수) : 연월 개월 수 이후의 연월을 구한다.(연월은 YYYY 또는 YYYYMM형식을 사용한다.)
-- PERIOD_DIFF(연월1, 연월2) :  연월1 - 연월2의 개월 수를 구한다.
